...
const CheckoutModal: React.FC<CheckoutModalProps> = ({ isOpen, onClose, plan, onOpenLogin }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!response.ok) {
                 const errorData = await response.json().catch(() => ({ message: 'Erro interno ao salvar assinatura.' }));
                throw new Error(errorData.message || 'Erro interno ao salvar assinatura.');
            }
            ...
            <div className="bg-[#FFF9F2] rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-8 relative" onClick={(e) => e.stopPropagation()}>
                ...
                {step === 'details' && (
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                <label htmlFor="flavor_preference" className="block text-sm font-medium text-[#5D4037]">Alguma preferência de sabor ou restrição? <span className="text-gray-400">(Opcional)</span></label>
                                <input type="text" name="flavor_preference" id="flavor_preference" value={flavorPreference} onChange={(e) => setFlavorPreference(e.target.value)} placeholder="Ex: Amo chocolate, sem nozes por favor" className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#D99A9A] focus:border-[#D99A9A]" />
                            </div>
                        ...
                        <button type="submit" className="w-full mt-8 py-3 px-6 rounded-lg font-semibold text-white bg-[#D99A9A] hover:bg-[#BF8B8B] transition-colors disabled:bg-gray-400">
                ...
                {step === 'success' && (
                    <div className="text-center py-10">
                        ...
                        <p className="text-lg text-gray-700 mb-6">Sua assinatura do plano <span className="font-bold">{plan.title}</span> foi confirmada. Em breve você receberá um email com os detalhes. Prepare-se para muito carinho em forma de bolo!</p>
